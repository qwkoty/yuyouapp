...
import { getMpConfig, saveMpConfig, deleteMpConfig, verifyWechatSignature, parseWechatXml, buildReplyXml, handleWechatMessage } from '../services/wechatMpService';
...
// 获取智能体的公众号配置
router.get('/agents/:id/wechat-mp', async (req, res) => {
  try {
    const config = await getMpConfig(req.params.id);
    // 不返回敏感字段
    if (config) {
      res.json({
        success: true,
...
// 保存/更新公众号配置
router.post('/agents/:id/wechat-mp', async (req, res) => {
  try {
    const { token, appId, appSecret, mpToken, encodingAesKey, isActive } = req.body;
    if (!token) { res.status(400).json({ error: '缺少token' }); return; }
    ...
    const agent = await getAgentById(req.params.id);
    if (!agent) { res.status(404).json({ error: '智能体不存在' }); return; }
    if (agent.user_id !== user.id) { res.status(403).json({ error: '无权限' }); return; }

    // 输入校验
    if (appId && !/^[a-zA-Z0-9]{10,20}$/.test(appId)) {
      res.status(400).json({ error: 'AppID 格式不正确' });
      return;
    }
    if (mpToken && (mpToken.length < 3 || mpToken.length > 32)) {
      res.status(400).json({ error: 'Token 长度需在 3-32 位之间' });
      return;
    }

    const config = await saveMpConfig(req.params.id, {
      appId,
      appSecret,
      token: mpToken,
      encodingAesKey,
...
// 删除公众号配置
router.delete('/agents/:id/wechat-mp', async (req, res) => {
  try {
    const { token } = req.body;
    if (!token) { res.status(400).json({ error: '缺少token' }); return; }
    ...
    await deleteMpConfig(req.params.id);
    res.json({ success: true });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
// 微信公众号消息接收接口（微信服务器回调）
// 路径格式: /api/wechat-callback/:agentId
router.get('/wechat-callback/:agentId', async (req, res) => {
  try {
...
router.post('/wechat-callback/:agentId', async (req, res) => {
  try {
    const { agentId } = req.params;
    ...
    if (!message) {
      res.status(400).send('消息解析失败');
      return;
    }

    // 处理消息并获取回复
    const reply = await handleWechatMessage(agentId, message);
